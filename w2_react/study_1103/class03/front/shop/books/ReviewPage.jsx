...
const ReviewPage = ({ location, setBook, book }) => {
    ...
    const onClickCancle = (rid, text, contents) => {
        if (text !== contents) {
            // if (!window.confirm("수정된 내용이 있습니다. 정말 취소하시겠습니까?")) return;
            setBox({
                show:true,
                message:"수정된 내용이 있습니다. 정말 취소하시겠습니까?",
                action: () => {
                            ...
                            {!review.edit ?
                                <>
                                    ...
                                    {review.uid === sessionStorage.getItem("uid") &&
                                        <div className='text-end mt-2'>
                                            <Button variant='danger' size='sm me-2'
                                                onClick={() => onClickDelete(review.rid)}>삭제</Button>
                                            <Button variant='primary' size='sm'
                                                onClick={() => onClickUpdate(review.rid)}>수정</Button>
                                        </div>
                                    }
                                </>
                                :
                                <>
                                    ...
                                    <div className='text-end mt-2'>
                                        <Button variant='primary' size='sm me-2'
                                            onClick={() => onClickSave(review.rid, review.text, review.contents)}>저장</Button>
                                        <Button variant='secondary' size='sm'
                                            onClick={() => onClickCancle(review.rid, review.text, review.contents)}>취소</Button>
                                    </div>
                                </>
                            }
                        </Col>
