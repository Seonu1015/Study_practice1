...
import { Spinner, Row, Col, Card, CardBody, InputGroup, Form, Button, CardFooter } from 'react-bootstrap';
            ...
            <Row className='mt-3'>
                {list.map(shop =>
                    <Col key={shop.pid} xs={6} md={4} lg={2} className='mb-3'>
                        <Card style={{cursor:"pointer"}} onClick={() => navi(`/shop/info/${shop.pid}`)}>
                            <CardBody>
                                <img src={`/display?file=${shop.image}`} width="100%" />
                                <div className='ellipsis'>{shop.title}</div>
                                <div className='text-end'>{shop.fmtprice}원</div>
                            </CardBody>
                            <CardFooter className='align-self-middle'>
                                <span><TbClick style={{fontSize:"100%"}} /> {shop.viewcnt}</span>
                                <span>
                                </span>
                            </CardFooter>
                        </Card>
