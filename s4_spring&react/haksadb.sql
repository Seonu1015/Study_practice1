...
select s.*, pname, date_format(birthday, '%Y-%m-%d') fmtdate
from students s, professors p
where advisor = pcode
order by scode desc;

select * from view_stu where pname like '%이재광%';
...
select *
from courses
where instructor = 221;

select * from courses;

create view stu_enroll as
select c.*, e.scode, e.edate, e.grade, pname
from enrollments e, courses c, professors p
where e.lcode = c.lcode and instructor = pcode;

select * from stu_enroll where scode = 92414029;

desc stu_enroll;

update courses c
set persons = 
(select count(scode) from enrollments
where lcode = c.lcode)
where c.lcode > '';
...
/*학생별 평균*/
select e.scode, sname, avg(grade) avg_grade
from enrollments e, students s
where e.scode = s.scode
group by e.scode
/*having avg_grade >= 80*/
